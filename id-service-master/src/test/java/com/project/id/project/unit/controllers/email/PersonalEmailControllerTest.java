...
import com.project.id.project.application.controllers.emails.PersonalEmailController;
...
public class PersonalEmailControllerTest {
    ...
    @InjectMocks
    private PersonalEmailController controller;
    ...
    void create_ShouldCallServiceAndReturnCreatedStatus() {
        ...
        // Assert
        verify(personalEmailService).create(USERNAME, dto);
    ...
    void read_ShouldReturnDto_WhenIdLinkedToUser() {
        ...
        when(personalEmailService.read(id)).thenReturn(Optional.of(dto));

        // Act
        ResponseEntity<PersonalEmailDTO> response = controller.read(id);
    ...
    void update_ShouldCallServiceAndReturnNoContent_WhenAuthorized() {
        ...
        // Assert
        verify(personalEmailService).update(id, dto, personalData.getId());
    ...
    void delete_ShouldCallServiceAndReturnNoContent_WhenAuthorized() {
        ...
        // Assert
        verify(personalEmailService).delete(id);
        assertEquals(HttpStatus.NO_CONTENT, response.getStatusCode());
    }
    ...
    private PersonalData createMockPersonalDataWithEmail(Id id) {
        PersonalEmail personalEmail = new PersonalEmail();
        personalEmail.setId(id);

        PersonalData personalData = new PersonalData();
        personalData.setPersonalEmail(personalEmail);
