...
import com.project.id.project.application.controllers.phonenumbers.PhoneNumberController;
...
public class PhoneNumberControllerTest {
    ...
    void create_ShouldCallServiceAndReturnCreatedStatus() {
        ...
        // Assert
        verify(phoneNumberService).create(USERNAME, dto);
    ...
    void read_ShouldReturnDto_WhenIdLinkedToUser() {
        ...
        when(phoneNumberService.read(id)).thenReturn(Optional.of(dto));

        // Act
        ResponseEntity<PhoneNumberDTO> response = controller.read(id);
    ...
    void update_ShouldCallServiceAndReturnNoContent_WhenAuthorized() {
        ...
        // Assert
        verify(phoneNumberService).update(id, dto, personalData.getId());
    ...
    void delete_ShouldCallServiceAndReturnNoContent_WhenAuthorized() {
        ...
        // Assert
        verify(phoneNumberService).delete(id);
        assertEquals(HttpStatus.NO_CONTENT, response.getStatusCode());
    }
